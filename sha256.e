...
EXPORT PROC calcSha256hex(text,outtext)
  DEF sha[32]:ARRAY OF CHAR
  DEF i

  calc_sha_256(sha,text,StrLen(text))
  StrCopy(outtext,'')
  FOR i:=0 TO 31
    StringF(outtext,'\s\r\z\h[2]',outtext,sha[i])
  ENDFOR
ENDPROC

EXPORT PROC calcSha1base32(text,outtext)
  DEF sha[32]:ARRAY OF CHAR
  DEF i
  DEF tempStr[255]:STRING
  DEF binaryStr[160]:STRING
  DEF base32chars:PTR TO CHAR
  
  base32chars:='ABCDEFGHIJKLMNOPQRSTUV0123456789'

  calc_sha_1(sha,text,StrLen(text))
  StrCopy(tempStr,'')
  FOR i:=0 TO 19
    StringF(tempStr,'\s\r\z\h[2]',tempStr,sha[i])
  ENDFOR
  StrCopy(binaryStr,'')
  FOR i:=0 TO 39
    SELECT tempStr[i]
      CASE "0"
        StrAdd(binaryStr,'0000')
      CASE "1"
        StrAdd(binaryStr,'0001')
      CASE "2"
        StrAdd(binaryStr,'0010')
      CASE "3"
        StrAdd(binaryStr,'0011')
      CASE "4"
        StrAdd(binaryStr,'0100')
      CASE "5"
        StrAdd(binaryStr,'0101')
      CASE "6"
        StrAdd(binaryStr,'0110')
      CASE "7"
        StrAdd(binaryStr,'0111')
      CASE "8"
        StrAdd(binaryStr,'1000')
      CASE "9"
        StrAdd(binaryStr,'1001')
      CASE "A"
        StrAdd(binaryStr,'1010')
      CASE "B"
        StrAdd(binaryStr,'1011')
      CASE "C"
        StrAdd(binaryStr,'1100')
      CASE "D"
        StrAdd(binaryStr,'1101')
      CASE "E"
        StrAdd(binaryStr,'1110')
      CASE "F"
        StrAdd(binaryStr,'1111')
    ENDSELECT
  ENDFOR
  StrCopy(outtext,'')
  FOR i:=0 TO 31
    StrCopy(tempStr,'%')
    StrAdd(tempStr,binaryStr+(i*5),5)
    StrAddChar(outtext,base32chars[Val(tempStr)])
  ENDFOR
ENDPROC
